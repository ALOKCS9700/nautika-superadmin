...
const PaymentDetails: React.FC = () => {
    ...
    useEffect(() => {
        const fetchPaymentDetails = async () => {
            try {
                const response = await axios.get('https://vertex-fx.onrender.com/user/payment-info');
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            try {
                const response = await axios.post('https://vertex-fx.onrender.com/admin/intro/upload-image', formData, {
                    headers: {
                ...
                if (response.data.fileUrl) {
                    setPaymentDetails({
                        ...paymentDetails,
                        QRImage: `https://vertex-fx.onrender.com${response.data.fileUrl}`
                    });
                } else {
    ...
    const handleSave = async () => {
        try {
            const response = await axios.post('https://vertex-fx.onrender.com/user/payment-info', paymentDetails);
