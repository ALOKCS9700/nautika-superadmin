...
const CommodityManagement: React.FC = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://vertex-fx.onrender.com/admin/commodities')
      .then(response => {
  ...
  const handleDelete = async () => {
    if (SelectedUserDelete._id) {
      try {
        const response = await fetch(`https://vertex-fx.onrender.com/admin/commodities/${SelectedUserDelete._id}`, {
          method: "DELETE",
        });
